...
The system SHALL capture a `ScopeManifest` containing normalized POSIX-relative include globs, exclude globs, allowed operations (`create`, `modify`, `delete`, `rename`), symlink policy, generated-file policy, and a canonical baseline identity before write-capable work begins. The canonical path grammar is `segment *("/" segment)`, where `segment = 1*(UTF8-NUL-free-unreserved / " " / "." / "-")` after NFC normalization; empty segments, `.`/`..`, NUL, backslash, drive prefixes, UNC prefixes, absolute paths, and invalid UTF-8 are rejected. Matching is case-sensitive. A valid glob is `pattern = segment *("/" segment)`, where each segment contains literals, `*` (zero or more non-`/` characters), or `?` (exactly one non-`/` character), and the segment `**` alone matches zero or more complete segments; `foo**bar`, `foo/**/`, and malformed escapes are rejected. Paths are slash-normalized, dot-segment-free, and sorted by UTF-8 bytes; excludes take precedence over includes, and a rename is represented as delete-old plus create-new unless rename is explicitly allowed. Symlink entries match the link path and target digest separately; traversal through a symlink is never implicit. The server SHALL derive changed paths from a canonical diff against that baseline, including tracked, staged, untracked, deleted, renamed, symlink, generated, and ignored-file changes according to the manifest policy; child-reported paths are evidence only. A result outside the manifest MUST be rejected deterministically with `scope_violation`, and its workspace MUST be retained for explicit user disposition.
...
- **WHEN** the manifest contains `src/**`, `src/*.py`, `foo**bar`, `foo/**/`, `C:/x`, `//host/share`, an empty segment, or a non-NFC equivalent path
- **THEN** `src/**` and `src/*.py` are valid, the other malformed/absolute patterns are rejected before execution, and equivalent NFC paths normalize to one bytewise identity
